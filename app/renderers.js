  ...
  return entries
    .map((entry) => `
      <div class="transcript-entry">
        <div class="transcript-meta">${entry.time} · ${entry.speaker}</div>
        <div>${entry.text}</div>
      </div>
    `)
    .join('');
  ...
  return entries
    .map((entry) => `
      <div class="activity-entry ${entry.isActive ? 'active' : ''}">
        <div class="activity-meta">${entry.time} · Triggered widgets: ${(entry.widgets || []).join(', ') || 'none'}</div>
        <div>${entry.summary || entry.text}</div>
      </div>
    `)
    .join('');
}
...
export function renderConcepts(concepts) {
  return concepts
    .map((concept) => `<button class="chip concept-chip-button" type="button" data-concept="${escapeAttr(concept)}">${concept}</button>`)
    .join('');
}
...
export function renderContextExplorer(contextUnderstanding) {
  ...
  return items
    .map((item) => `<button class="chip concept-chip-button context-chip-${item.kind}" type="button" data-context-item="${escapeAttr(item.label)}" data-context-kind="${item.kind}">${item.label}</button>`)
    .join('');
}
